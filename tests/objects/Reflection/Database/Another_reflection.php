<?php 
/**
 * @noinspection KphpReturnTypeMismatchInspection
 * @noinspection NoTypeDeclarationInspection
 * @noinspection PhpFullyQualifiedNameUsageInspection
 * @noinspection PhpUnusedParameterInspection
 * @noinspection PhpIllegalPsrClassPathInspection
 * @noinspection DegradedSwitchInspection 
 */
namespace Database;



use ADO\src\Sigmalab\Database\Core\DBSettings;
use Sigmalab\Database\DatabaseArgumentException;
use Sigmalab\Database\Expressions\ExprEQ;
use Sigmalab\Database\Expressions\IExpression;
use Sigmalab\Database\Sql\SQLJoin;
use Sigmalab\IsKPHP;
use Sigmalab\SimpleReflection\ClassRegistry;
use Sigmalab\SimpleReflection\ICanReflection;
use Sigmalab\SimpleReflection\PhpReflection;

use \Sigmalab\SimpleReflection\IReflectedObject;
use \Sigmalab\SimpleReflection\ReflectionException;

//autogenerated
class Another_reflection implements \Sigmalab\SimpleReflection\ICanReflection
{
	/** Another */
	private Another $instance;

	public const Field_another_link_id = "another_link_id";
	public const Field_owner_id = "owner_id";
	public const Field_child_id = "child_id";

	
	public function __construct(Another $instance) 
	{
		$this->instance = $instance;
	}	
	/**
	 * @kphp-required
	 * @param string $name
	 * @return bool
	 */
	public function isPropertyExists(string $name):bool
	{
		//autogenerated map
		switch ($name) {
			case self::Field_another_link_id: return true;
			case self::Field_owner_id: return true;
			case self::Field_child_id: return true;
		}
		return false;
	}
	
	/**
	 * @kphp-required
	 * @return string[]
	 *
	 */
	public function getProperties() : array
	{
		return [self::Field_another_link_id,
		self::Field_owner_id,
		self::Field_child_id];
	}
	
	/**
	 * @kphp-required
	 * @param string $name
	 * @return \Sigmalab\SimpleReflection\TypeName
	 *
	 */
	public function getPropertyType(string $name) : \Sigmalab\SimpleReflection\TypeName
	{
		//autogenerated map
		switch ($name) {
			case self::Field_another_link_id: return \Sigmalab\SimpleReflection\TypeName::$intValue;
			case self::Field_owner_id: return \Sigmalab\SimpleReflection\TypeName::$intValue;
			case self::Field_child_id: return \Sigmalab\SimpleReflection\TypeName::$intValue;
			default:
				//throw new ReflectionException("Property '{$name}' not found in Another"); 
		}
		return \Sigmalab\SimpleReflection\TypeName::$intValue;
	}
	

	/**
	 * @kphp-required
	 * @param string $name
	 * @param \Sigmalab\SimpleReflection\ValueMixed $value
	 */
	public function setPropertyValue(string $name, \Sigmalab\SimpleReflection\ValueMixed $value) : void
	{
		//autogenerated map
		switch ($name) {
/* int: int type:int isArray:  */
		case self::Field_another_link_id:
			if ($value instanceof \Sigmalab\SimpleReflection\ValueScalar) {
				$this->instance->another_link_id = $value->get_as_int();
			}

			break;
/* int: int type:int isArray:  */
		case self::Field_owner_id:
			if ($value instanceof \Sigmalab\SimpleReflection\ValueScalar) {
				$this->instance->owner_id = $value->get_as_int();
			}

			break;
/* int: int type:int isArray:  */
		case self::Field_child_id:
			if ($value instanceof \Sigmalab\SimpleReflection\ValueScalar) {
				$this->instance->child_id = $value->get_as_int();
			}

			break;
			default:
				//throw new ReflectionException("Property '{$name}' not found in Another"); 
		}
	}

	/**
	 * @kphp-required
	 * @param string $name
	 * @return \Sigmalab\SimpleReflection\ValueMixed
	 */
	public function getPropertyValue(string $name) :\Sigmalab\SimpleReflection\ValueMixed
	{
		//autogenerated map
		switch ($name) {
			case self::Field_another_link_id: return new \Sigmalab\SimpleReflection\ValueScalar( $this->instance->another_link_id );
			case self::Field_owner_id: return new \Sigmalab\SimpleReflection\ValueScalar( $this->instance->owner_id );
			case self::Field_child_id: return new \Sigmalab\SimpleReflection\ValueScalar( $this->instance->child_id );
			default:
				//throw new ReflectionException("Property '$name' not found in Another");

				
		}
		return null;
	}
	
	/**  execute register class via reflection class instance */
	public function dynamicRegister(){
		self::registerClass();
	}
	/** @noinspection PhpFullyQualifiedNameUsageInspection */
	public static function registerClass()
	{   
		\Sigmalab\SimpleReflection\ClassRegistry::registerClass(Another::class, function(){
			return instance_cast(new Another, IReflectedObject::class ); 
		});
		\Sigmalab\SimpleReflection\ClassRegistry::registerReflection(self::class, function(object $instance){
			/** @noinspection PhpParamsInspection */ 
			return new Another_reflection(instance_cast($instance, Another::class)); 
		});
		
	}
	

	/**
	* @inheritDoc
	*/
	public function invoke_as_mixed(string $methodName, array $args) 
	{
		switch ($methodName)
		{
 //type: int 
			case 'primary_key_value':
				return $this->invoke_primary_key_value($args);
				break;
 //type: string 
			case 'table_name':
				return $this->invoke_table_name($args);
				break;
 //type: string 
			case 'table_description':
				return $this->invoke_table_description($args);
				break;
 //type: bool 
			case 'isNew':
				return $this->invoke_isNew($args);
				break;
 //type: bool 
			case 'set_another_link_id':
				return $this->invoke_set_another_link_id($args);
				break;
 //type: int 
			case 'get_another_link_id':
				return $this->invoke_get_another_link_id($args);
				break;
 //type: bool 
			case 'set_owner_id':
				return $this->invoke_set_owner_id($args);
				break;
 //type: int 
			case 'get_owner_id':
				return $this->invoke_get_owner_id($args);
				break;
 //type: bool 
			case 'set_child_id':
				return $this->invoke_set_child_id($args);
				break;
 //type: int 
			case 'get_child_id':
				return $this->invoke_get_child_id($args);
				break;
 //type: bool 
			case 'importValueByKey':
				return $this->invoke_importValueByKey($args);
				break;
 //type: bool 
			case 'setValue':
				return $this->invoke_setValue($args);
				break;
			default:
				//throw new ReflectionException("Property '$methodName' not found in Another");

				
		}
	}
	/**
	* @inheritDoc
	*/
	public function invoke_as_object(string $methodName, array $args) :?IReflectedObject
	{
		return null;
	}

	/**
	 * @param string $methodName
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 */
	public function invoke_as_void(string $methodName, array $args) :void
	{
		switch ($methodName)
		{
			case 'getAnotherPrototype':
				$this->invoke_getAnotherPrototype($args);
				break;
			case 'getPrimaryKeyTag':
				$this->invoke_getPrimaryKeyTag($args);
				break;
			case 'primary_key_value':
				$this->invoke_primary_key_value($args);
				break;
			case 'set_primary_key_value':
				$this->invoke_set_primary_key_value($args);
				break;
			case 'createSelf':
				$this->invoke_createSelf($args);
				break;
			case 'table_name':
				$this->invoke_table_name($args);
				break;
			case 'table_description':
				$this->invoke_table_description($args);
				break;
			case 'getColumnDefinition':
				$this->invoke_getColumnDefinition($args);
				break;
			case 'getForeignKeys':
				$this->invoke_getForeignKeys($args);
				break;
			case 'isNew':
				$this->invoke_isNew($args);
				break;
			case 'set_another_link_id':
				$this->invoke_set_another_link_id($args);
				break;
			case 'get_another_link_id':
				$this->invoke_get_another_link_id($args);
				break;
			case 'set_owner_id':
				$this->invoke_set_owner_id($args);
				break;
			case 'get_owner_id':
				$this->invoke_get_owner_id($args);
				break;
			case 'set_child_id':
				$this->invoke_set_child_id($args);
				break;
			case 'get_child_id':
				$this->invoke_get_child_id($args);
				break;
			case 'importValue':
				$this->invoke_importValue($args);
				break;
			case 'importValueByKey':
				$this->invoke_importValueByKey($args);
				break;
			case 'setValue':
				$this->invoke_setValue($args);
				break;
			case 'getValue':
				$this->invoke_getValue($args);
				break;
			case 'tag_another_link_id':
				$this->invoke_tag_another_link_id($args);
				break;
			case 'tag_owner_id':
				$this->invoke_tag_owner_id($args);
				break;
			case 'tag_child_id':
				$this->invoke_tag_child_id($args);
				break;
			case 'prototypeMap':
				$this->invoke_prototypeMap($args);
				break;
			case 'key_owner_id':
				$this->invoke_key_owner_id($args);
				break;
			case 'key_child_id':
				$this->invoke_key_child_id($args);
				break;
			default:
				//throw new ReflectionException("Property '$methodName' not found in Another");

				
		}
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_getAnotherPrototype(array $args)
	{
//type: Parameter #0 [ <optional> string or NULL $alias = NULL ]
//param: string

		/** @var string $arg0  simple type string*/      
		$value0 = $args[0];
		if ( $value0 instanceof \Sigmalab\SimpleReflection\ValueScalar) $arg0 = $value0->get_as_string();
		else throw new ReflectionException("Property '0' not found in Another");

//return: Database\Another
		$this->instance->getAnotherPrototype($arg0);
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_getPrimaryKeyTag(array $args)
	{
//return: Sigmalab\Database\Core\IDBColumnDefinition
		$this->instance->getPrimaryKeyTag();
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_primary_key_value(array $args)
	{
//return: int
		return $this->instance->primary_key_value();
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_set_primary_key_value(array $args)
	{
//type: Parameter #0 [ <required> int $value ]
//param: int

		/** @var int $arg0  simple type int*/      
		$value0 = $args[0];
		if ( $value0 instanceof \Sigmalab\SimpleReflection\ValueScalar) $arg0 = $value0->get_as_int();
		else throw new ReflectionException("Property '0' not found in Another");

//return: void
		$this->instance->set_primary_key_value($arg0);
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_createSelf(array $args)
	{
//return: Sigmalab\Database\Core\IDataObject
		$this->instance->createSelf();
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_table_name(array $args)
	{
//return: string
		return $this->instance->table_name();
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_table_description(array $args)
	{
//return: string
		return $this->instance->table_description();
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_getColumnDefinition(array $args)
	{
//return: array
		return $this->instance->getColumnDefinition();
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_getForeignKeys(array $args)
	{
//return: array
		return $this->instance->getForeignKeys();
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_isNew(array $args)
	{
//return: bool
		return $this->instance->isNew();
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_set_another_link_id(array $args)
	{
//type: Parameter #0 [ <required> int or NULL $value ]
//param: int

		/** @var int $arg0  simple type int*/      
		$value0 = $args[0];
		if ( $value0 instanceof \Sigmalab\SimpleReflection\ValueScalar) $arg0 = $value0->get_as_int();
		else throw new ReflectionException("Property '0' not found in Another");

//return: bool
		return $this->instance->set_another_link_id($arg0);
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_get_another_link_id(array $args)
	{
//return: int
		return $this->instance->get_another_link_id();
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_set_owner_id(array $args)
	{
//type: Parameter #0 [ <required> int or NULL $value ]
//param: int

		/** @var int $arg0  simple type int*/      
		$value0 = $args[0];
		if ( $value0 instanceof \Sigmalab\SimpleReflection\ValueScalar) $arg0 = $value0->get_as_int();
		else throw new ReflectionException("Property '0' not found in Another");

//return: bool
		return $this->instance->set_owner_id($arg0);
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_get_owner_id(array $args)
	{
//return: int
		return $this->instance->get_owner_id();
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_set_child_id(array $args)
	{
//type: Parameter #0 [ <required> int or NULL $value ]
//param: int

		/** @var int $arg0  simple type int*/      
		$value0 = $args[0];
		if ( $value0 instanceof \Sigmalab\SimpleReflection\ValueScalar) $arg0 = $value0->get_as_int();
		else throw new ReflectionException("Property '0' not found in Another");

//return: bool
		return $this->instance->set_child_id($arg0);
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_get_child_id(array $args)
	{
//return: int
		return $this->instance->get_child_id();
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_importValue(array $args)
	{
//type: Parameter #0 [ <required> string $name ]
//param: string

		/** @var string $arg0  simple type string*/      
		$value0 = $args[0];
		if ( $value0 instanceof \Sigmalab\SimpleReflection\ValueScalar) $arg0 = $value0->get_as_string();
		else throw new ReflectionException("Property '0' not found in Another");

//type: Parameter #1 [ <required> $value ]
//param: mixed

		/** @var mixed $arg1  - mixed */      
		$value1 = $args[1];
		if ( $value1 instanceof \Sigmalab\SimpleReflection\ValueScalar) $arg1 = $value1->get_as_mixed();
		else throw new ReflectionException("Property '1' not found in Another");

//return: void
		$this->instance->importValue($arg0,$arg1);
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_importValueByKey(array $args)
	{
//type: Parameter #0 [ <required> int $key ]
//param: int

		/** @var int $arg0  simple type int*/      
		$value0 = $args[0];
		if ( $value0 instanceof \Sigmalab\SimpleReflection\ValueScalar) $arg0 = $value0->get_as_int();
		else throw new ReflectionException("Property '0' not found in Another");

//type: Parameter #1 [ <required> $value ]
//param: mixed

		/** @var mixed $arg1  - mixed */      
		$value1 = $args[1];
		if ( $value1 instanceof \Sigmalab\SimpleReflection\ValueScalar) $arg1 = $value1->get_as_mixed();
		else throw new ReflectionException("Property '1' not found in Another");

//return: bool
		return $this->instance->importValueByKey($arg0,$arg1);
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_setValue(array $args)
	{
//type: Parameter #0 [ <required> string $name ]
//param: string

		/** @var string $arg0  simple type string*/      
		$value0 = $args[0];
		if ( $value0 instanceof \Sigmalab\SimpleReflection\ValueScalar) $arg0 = $value0->get_as_string();
		else throw new ReflectionException("Property '0' not found in Another");

//type: Parameter #1 [ <required> $value ]
//param: mixed

		/** @var mixed $arg1  - mixed */      
		$value1 = $args[1];
		if ( $value1 instanceof \Sigmalab\SimpleReflection\ValueScalar) $arg1 = $value1->get_as_mixed();
		else throw new ReflectionException("Property '1' not found in Another");

//return: bool
		return $this->instance->setValue($arg0,$arg1);
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_getValue(array $args)
	{
//type: Parameter #0 [ <required> string $name ]
//param: string

		/** @var string $arg0  simple type string*/      
		$value0 = $args[0];
		if ( $value0 instanceof \Sigmalab\SimpleReflection\ValueScalar) $arg0 = $value0->get_as_string();
		else throw new ReflectionException("Property '0' not found in Another");

//return: 
		$this->instance->getValue($arg0);
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_tag_another_link_id(array $args)
	{
//type: Parameter #0 [ <optional> string or NULL $alias = NULL ]
//param: string

		/** @var string $arg0  simple type string*/      
		$value0 = $args[0];
		if ( $value0 instanceof \Sigmalab\SimpleReflection\ValueScalar) $arg0 = $value0->get_as_string();
		else throw new ReflectionException("Property '0' not found in Another");

//return: Sigmalab\Database\Core\IDBColumnDefinition
		$this->instance->tag_another_link_id($arg0);
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_tag_owner_id(array $args)
	{
//type: Parameter #0 [ <optional> string or NULL $alias = NULL ]
//param: string

		/** @var string $arg0  simple type string*/      
		$value0 = $args[0];
		if ( $value0 instanceof \Sigmalab\SimpleReflection\ValueScalar) $arg0 = $value0->get_as_string();
		else throw new ReflectionException("Property '0' not found in Another");

//return: Sigmalab\Database\Core\IDBColumnDefinition
		$this->instance->tag_owner_id($arg0);
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_tag_child_id(array $args)
	{
//type: Parameter #0 [ <optional> string or NULL $alias = NULL ]
//param: string

		/** @var string $arg0  simple type string*/      
		$value0 = $args[0];
		if ( $value0 instanceof \Sigmalab\SimpleReflection\ValueScalar) $arg0 = $value0->get_as_string();
		else throw new ReflectionException("Property '0' not found in Another");

//return: Sigmalab\Database\Core\IDBColumnDefinition
		$this->instance->tag_child_id($arg0);
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_prototypeMap(array $args)
	{
//return: array
		return $this->instance->prototypeMap();
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_key_owner_id(array $args)
	{
//type: Parameter #0 [ <optional> Database\Data or NULL $proto = NULL ]
//param: \Database\Data

		/** @var \Database\Data $arg0 - object  */      
		$value0 = $args[0];
		if ( $value0 instanceof \Sigmalab\SimpleReflection\ValueObject) $arg0 = instance_cast($value0, \Database\Data::class);
		else throw new ReflectionException("Property '0' not found in Another");

//return: Sigmalab\Database\Core\DBForeignKey
		$this->instance->key_owner_id($arg0);
	}

	/**
	 * @param \Sigmalab\SimpleReflection\ValueMixed[] $args
	 * @throws \Exception
	 * 
	 */
	private function invoke_key_child_id(array $args)
	{
//type: Parameter #0 [ <optional> Database\Dictionary or NULL $proto = NULL ]
//param: \Database\Dictionary

		/** @var \Database\Dictionary $arg0 - object  */      
		$value0 = $args[0];
		if ( $value0 instanceof \Sigmalab\SimpleReflection\ValueObject) $arg0 = instance_cast($value0, \Database\Dictionary::class);
		else throw new ReflectionException("Property '0' not found in Another");

//return: Sigmalab\Database\Core\DBForeignKey
		$this->instance->key_child_id($arg0);
	}
	/**
	 * @param string $name
	 * @param string $value
	 * @throws \Exception
	 */
	public function set_as_string(string  $name, string $value):void
	{
		switch ($name) {
			default: 
			//throw new ReflectionException("Property '$name' not found in \Database\Another");
		}
	}
	/**
	 * @param string $name
	 * @return string
	 * @throws \Exception
	 */
	public function get_as_string(string  $name):string
	{
		switch ($name) {
			default: 
			//throw new ReflectionException("Property '$name' not found in \Database\Another");
		}
		/** @noinspection PhpUnreachableStatementInspection */
		return "";
	}
	/**
	 * @param string $name
	 * @param int $value
	 * @throws \Exception
	 */
	public function set_as_int(string  $name, int $value):void
	{
		switch ($name) {
			case self::Field_another_link_id: $this->instance->another_link_id  = $value; break;
			case self::Field_owner_id: $this->instance->owner_id  = $value; break;
			case self::Field_child_id: $this->instance->child_id  = $value; break;
			default: 
			//throw new ReflectionException("Property '$name' not found in \Database\Another");
		}
	}
	/**
	 * @param string $name
	 * @return int
	 * @throws \Exception
	 */
	public function get_as_int(string  $name):int
	{
		switch ($name) {
			case self::Field_another_link_id: return (int)$this->instance->another_link_id;
			case self::Field_owner_id: return (int)$this->instance->owner_id;
			case self::Field_child_id: return (int)$this->instance->child_id;
			default: 
			//throw new ReflectionException("Property '$name' not found in \Database\Another");
		}
		/** @noinspection PhpUnreachableStatementInspection */
		return 0;
	}
	/**
	 * @param string $name
	 * @param float $value
	 * @throws \Exception
	 */
	public function set_as_float(string  $name, float $value):void
	{
		switch ($name) {
			default: 
			//throw new ReflectionException("Property '$name' not found in \Database\Another");
		}
	}
	/**
	 * @param string $name
	 * @return float
	 * @throws \Exception
	 */
	public function get_as_float(string  $name):float
	{
		switch ($name) {
			default: 
			//throw new ReflectionException("Property '$name' not found in \Database\Another");
		}
		/** @noinspection PhpUnreachableStatementInspection */
		return 0.0;
	}
	/**
	 * @param string $name
	 * @param bool $value
	 * @throws \Exception
	 */
	public function set_as_bool(string  $name, bool $value):void
	{
		switch ($name) {
			default: 
			//throw new ReflectionException("Property '$name' not found in \Database\Another");
		}
	}
	/**
	 * @param string $name
	 * @return bool
	 * @throws \Exception
	 */
	public function get_as_bool(string  $name):bool
	{
		switch ($name) {
			default: 
			//throw new ReflectionException("Property '$name' not found in \Database\Another");
		}
		/** @noinspection PhpUnreachableStatementInspection */
		return false;
	}
	/**
	 * @param string $name
	 * @throws \Exception
	 */
	public function set_as_null(string  $name):void
	{
		switch ($name) {
			case self::Field_another_link_id: $this->instance->another_link_id  = null; break;
			case self::Field_owner_id: $this->instance->owner_id  = null; break;
			case self::Field_child_id: $this->instance->child_id  = null; break;
			default: 
			//throw new ReflectionException("Property '$name' not found in \Database\Another");
		}
	}
	/**
	 * @param string $name
	 * @param mixed $value
	 * @throws \Exception
	 */
	public function set_as_mixed(string $name, $value):void
	{
		switch ($name) {
			case self::Field_another_link_id: 
				if ( is_null($value) ) $this->instance->another_link_id = null;
				else $this->instance->another_link_id  = (int)$value; 
				break;
			case self::Field_owner_id: 
				if ( is_null($value) ) $this->instance->owner_id = null;
				else $this->instance->owner_id  = (int)$value; 
				break;
			case self::Field_child_id: 
				if ( is_null($value) ) $this->instance->child_id = null;
				else $this->instance->child_id  = (int)$value; 
				break;
			default: 
			//throw new ReflectionException("Property '$name' not found in \Database\Another");
		}
	}
	/**
	 * @param string $name
	 * @return mixed
	 * @throws \Exception
	 */
	public function get_as_mixed(string $name)
	{
		switch ($name) {
			case self::Field_another_link_id: return $this->instance->another_link_id; break;
			case self::Field_owner_id: return $this->instance->owner_id; break;
			case self::Field_child_id: return $this->instance->child_id; break;
			default: 
			//throw new ReflectionException("Property '$name' not found in \Database\Another");
		}
		/** @noinspection PhpUnreachableStatementInspection */
		return 0;
	}
	/**
	 * @param string $name
	 * @param \Sigmalab\SimpleReflection\IReflectedObject $value
	 * @throws \Exception
	 */
	public function set_as_object(string $name, \Sigmalab\SimpleReflection\IReflectedObject $value):void
	{
		switch ($name) {
			default: 
			//throw new ReflectionException("Property '$name' not found in \Database\Another");
		}
	}

	/**
	 * @param string $name
	 * @return \Sigmalab\SimpleReflection\IReflectedObject|null 
	 * @throws \Exception
	 */
	public function get_as_object(string $name):?\Sigmalab\SimpleReflection\IReflectedObject 
	{
		switch ($name) {
			default: 
			//throw new ReflectionException("Property '$name' not found in \Database\Another");
		}
		/** @noinspection PhpUnreachableStatementInspection */
		return null;
	}

	/**
	 * @param string $name
	 * @param \Sigmalab\SimpleReflection\IReflectedObject[] $value
	 * @throws \Exception
	 */
	public function set_as_objects(string $name, array $value):void
	{
		switch ($name) {
			default: 
			//throw new ReflectionException("Property '$name' not found in \Database\Another");
		}
	}

	/**
	 * @param string $name
	 * @return \Sigmalab\SimpleReflection\IReflectedObject[] 
	 * @throws \Exception
	 */
	public function get_as_objects(string $name):array
	{
		switch ($name) {
			default: 
			//throw new ReflectionException("Property '$name' not found in \Database\Another");
		}
		/** @noinspection PhpUnreachableStatementInspection */
		return [];
	}
}
