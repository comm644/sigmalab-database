...
<!--
  <xsl:apply-templates select="column[@auto-increment ='yes']" mode="auto-increment">
     <xsl:with-param name="table" select="$table"/>
  </xsl:apply-templates>
...
Warning: Enums not supported. changed to 'string' value.
        ...
        <xsl:when test="@type = 'text'">BLOB</xsl:when>
        <xsl:when test="@type = 'varchar'">
          <xsl:if test="count(@length) = 0">
            <xsl:message terminate="yes">GenSql: Error: for column <xsl:value-of select="concat($table, '/', @name)"/> @length is not defined</xsl:message>
          </xsl:if>
  ...
  <xsl:template match="column" mode="foreign-key">
    ...
    <xsl:variable name="on-delete">
      <xsl:choose>
        <xsl:when test="@on-delete ='restrict'">NO ACTION</xsl:when>
        <xsl:otherwise><xsl:value-of select="@on-delete"/></xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
  ...
  <xsl:template match="column" mode="auto-increment">
    <xsl:param name="table"/>
    <xsl:variable name="this" select="."/>
    <xsl:variable name="generator" select="concat('gen_', $table, '_', $this/@name)"/>

CREATE GENERATOR <xsl:value-of select="$generator"/>;
SET GENERATOR <xsl:value-of select="$generator"/> TO 0;
set term !! ;
CREATE TRIGGER <xsl:value-of select="concat('bi_', $table, '_', @name)"/> FOR <xsl:value-of select="@name"/>
ACTIVE BEFORE INSERT POSITION 0
AS
BEGIN
   if (NEW.<xsl:value-of select="$this/@name"/> is NULL) then NEW.<xsl:value-of select="$this/@name"/> = GEN_ID(<xsl:value-of select="$generator"/>, 1)
END!!
set term ; !!
