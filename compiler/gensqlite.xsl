...
CREATE TABLE <xsl:value-of select="@name"/> (
  ...
  <xsl:apply-templates select="." mode="insert-data"/>
    ...
    <xsl:apply-templates select="index" mode="insert-index">
      <xsl:with-param name="table" select="$table"/>
    </xsl:apply-templates>
...
Warning: Enums not supported. changed to 'string' value.
        ...
        <xsl:when test="@type = 'jsonb'">text</xsl:when>
        <xsl:otherwise>
    ...
    <xsl:if test="@type ='text' or @type='varchar'"> COLLATE NOCASE</xsl:if>
  </xsl:template>
  ...
  <xsl:template match="index" mode="insert-index">
    <xsl:param name="table"/>
    <xsl:variable name="name">
      <xsl:for-each select="column">_<xsl:value-of select="@name"/></xsl:for-each>
    </xsl:variable>
    ...
    <xsl:text>CREATE INDEX IF NOT EXISTS</xsl:text> ix_<xsl:value-of select="$table"/><xsl:value-of select="$name"/> on <xsl:value-of select="$table"/>(<xsl:value-of select="$column"/>);<xsl:text>
</xsl:text>
  </xsl:template>
  ...
  ,</xsl:text>CONSTRAINT c_<xsl:value-of select="$table"/>_<xsl:value-of select="@name"/> FOREIGN KEY(<xsl:value-of select="@name"/>)
    ...
    <xsl:if test="count(@on-delete) != 0"> ON DELETE <xsl:value-of select="@on-delete"/></xsl:if>
  </xsl:template>
